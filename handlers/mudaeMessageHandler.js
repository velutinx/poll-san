...
function initMudaeMessageHandler(client) {
    ...
    client.on('messageCreate', async (message) => {
        ...
        setTimeout(() => {
            ...
            if (pendingClaims.has(key) && pendingClaims.get(key).messageId === message.id) {
    ...
    client.on('messageCreate', async (message) => {
        ...
        const pending = pendingClaims.get(normalizeName(characterName));
        ...
        if (pending) {
            pendingClaims.delete(normalizeName(characterName));
        }
    });
