...
const ROLL_LIFETIME_MS = 2 * 60 * 1000;    // 2 minutes per message
const CLAIM_LOOKUP_TIMEOUT_MS = 1 * 60 * 1000;
const INACTIVITY_PURGE_MS = 5 * 60 * 1000; // 5 minutes inactivity -> full purge

let inactivityTimeout = null;              // timeout for channel purge
...
function parseRollEmbed(embed) { /* unchanged */ }
...
function initMudaeMessageHandler(client) {
    ...
    function resetInactivityTimer(channel) {
        ...
        inactivityTimeout = setTimeout(async () => {
            try {
                ...
                const toDelete = messages.filter(msg => !WHITELISTED_MESSAGE_IDS.has(msg.id));
                if (toDelete.size === 0) return;
                await channel.bulkDelete(toDelete);
                console.log(`🧹 Purged ${toDelete.size} messages from ${channel.name} due to inactivity.`);
                // Clear per-message timeouts for deleted messages
    ...
    // ---- General message handler: schedule per-message deletion & reset inactivity timer ----
    client.on('messageCreate', async (message) => {
        if (message.channel.id !== helpers.ids.channels.mudae_roll) return;

        // Reset inactivity timer on ANY message (including whitelisted)
        resetInactivityTimer(message.channel);

        // If message is whitelisted, do NOT schedule per-message deletion
        if (WHITELISTED_MESSAGE_IDS.has(message.id)) return;

        // Schedule per-message deletion
    ...
    // ---- Roll detection & reaction (unchanged) ----
    client.on('messageCreate', async (message) => {
        ...
        pendingClaims.set(character, { series, messageId: message.id, timestamp: Date.now() });
        setTimeout(() => {
