...
async function cleanupExpiredMemberships(client) {
  try {
    const now = new Date();
    // ─── Clean up memberships that expired more than 14 days ago ───
    // This is longer than the 10‑day grace period, so records remain
    // during the grace window and are removed after.
    ...
    const expired = await db.query(
      `SELECT * FROM ${h.tables.MEMBERSHIPS}
       ...
       ORDER BY discord_id`,
      [now.toISOString(), fourteenDaysAgo.toISOString()]
    );
