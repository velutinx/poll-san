...
const UNVERIFIED_ROLE = h.ids.roles.unverified; // add this
...
module.exports = async function ensureAccessRoles(guild) {
    try {
        ...
        for (const member of members.values()) {
            // 1. Ignore bots completely
            if (member.user.bot) continue;

            // 2. Never touch the Creator
            if (member.roles.cache.has(CREATOR_ROLE)) continue;

            // 3. Never touch unverified users – they must verify first
            if (member.roles.cache.has(UNVERIFIED_ROLE)) continue;
